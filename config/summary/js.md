...
主线程先执行执行栈中的同步代码，遇到异步事件时，会把异步事件（宏任务或者微任务）挂载到任务队列中（task queue），任务队列分宏任务和微任务队列，存贮异步事件的回调。等到所有的同步代码都执行完成之后，再去任务队列中查看是否任务（先微后宏），如果有就进入执行栈，执行异步事件的回调函数。等任务栈中的同步代码执行完毕，再去任务队列中查看并执行异步事件，反复循环。
